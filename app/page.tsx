...
export default function Home() {
  ...
  const [backgroundType, setBackgroundType] = useState("video") // "image" or "video"
  const [backgroundUrl, setBackgroundUrl] = useState("/landing.mp4")
    ...
    async function fetchData() {
      try {
        ...
        if (backgroundData.background) {
          setBackgroundType(backgroundData.background.type || "video")
          setBackgroundUrl(backgroundData.background.url || "/landing.mp4")
        }
  ...
  useEffect(() => {
    if (videoRef.current && backgroundType === "video") {
